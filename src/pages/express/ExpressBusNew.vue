<template>
  ...
  >
    <form class="form-card" @submit.prevent="handleSubmit">
      <router-link to="/express/buses" class="back-link">
      ...
      <div class="form-actions">
        <router-link to="/express/buses" class="btn-secondary">Cancel</router-link>
...
const form = reactive({
  ...
  depotId: '', insuranceExpiry: '', nextServiceDate: '', amenities: [],
})
...
async function handleSubmit() {
  ...
  try {
    ...
    await auditLogService.log(
      ...
      'create', 'bus', bus.id,
      `Added bus ${form.plateNumber}`
    )

    router.push('/express/buses')
  } catch {
